...
all_results = []
if len(result_files)==relevant_time_windows.size:
    results = {}
    for i in range(relevant_time_windows.size):
        all_results.append(pd.read_csv(result_files[i]))
else: 
    raise IndexError('Num result files dont match the time poitns')
#all_results = [pd.read_csv(each) for each in result_files]
...
sensible_posns = np.argwhere(distmat_flighttraj<=0.15)
