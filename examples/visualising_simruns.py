...
# -*- coding: utf-8 -*-
"""
Effect of changing initial vertex
=================================
...
NBAT=8
output_data_pattern = 'K6*'
output_folder = os.path.join('initialvertex_tests',f'nbat{NBAT}')
arraygeom_file = os.path.join(output_folder,'mic_xyz_multibatsim.csv')
audiofile = os.path.join(output_folder,f'{NBAT}-bats_trajectory_simulation_raytracing-1.wav')
array_geom = pd.read_csv(arraygeom_file).loc[:,'x':'z'].to_numpy()
vsound = 340.0 # m/s
#%%
# load all the results into a dictionary
result_files = natsorted(glob.glob(os.path.join(output_folder,f'nbats{NBAT}outdata',f'*{output_data_pattern}*.csv')))
...
flight_traj = pd.read_csv(os.path.join(output_folder,'multibatsim_xyz_calling.csv'))
...
for i,(batid, source_prof) in enumerate(counts_by_batid.items()):
    ...
    pks, _ = signal.find_peaks(source_prof, distance=10,  height=4)
